%ZISC ;SFISC/GFT,AC,MUS - CLOSE LOGIC FOR DEVICES  ;11/08/2011
 ;;8.0;KERNEL;**24,36,49,69,199,216,275,409,440,585**;JUL 10, 1995;Build 4
...
C0 ;
 ...
 . ; DSS/SMH - BEGIN MODS - Move this from 3 lines down. Only needs to happen when a device is really closed.
 . ; DSS/SMH - Also, pass IO as an array, not a scalar
 ...
 ;
 ;**P585 START CJM
 ; DSS/SMH - BEGIN MODS - comment out moved up line
 ; I $G(IOT)="PQ" D CLOSE^ZISPQ(IO)
 ; DSS/SMH - END MODS
 ;**P585 END CJM
 ;
...
END I '$D(IO("T")),$G(%ZISPCX)]"" S %Y=%ZISPCX D %Y^ZISX
 ; DSS/SMH - BEGIN MODS - Delete post close execute after it's done
 I $D(IO("CLOSE")) K %ZISPCX,%Y
 ; DSS/SMH - END MODS
